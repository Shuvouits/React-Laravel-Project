...
class AdminOrderController extends Controller
{
    ...
    public function storeManual(
        ...
        $order = DB::transaction(function () use ($validated) {
            ...
            $paymentDueLater = (bool) (
                $validated['payment_due_later'] ?? false
            );
    ...
    public function refundReturn(
        ...
        if (
            ...
            return response()->json([
                'success' => false,
                'message' =>
                'This return has already been fully refunded.',
            ], 422);
        }
        ...
        if ($returnValue <= 0) {
            return response()->json([
                'success' => false,
                'message' =>
                'Unable to calculate the return value.',
            ], 422);
        }
        ...
        if ($remainingReturnAmount <= 0) {
            return response()->json([
                'success' => false,
                'message' =>
                'This return has no refundable balance remaining.',
            ], 422);
        }
        ...
        if ($amount > $remainingReturnAmount) {
            return response()->json([
                'success' => false,
                'message' =>
                'Refund amount cannot exceed the remaining return value.',
            ], 422);
        }
        ...
        if ($amount > $orderRefundableAmount) {
            return response()->json([
                'success' => false,
                'message' =>
                'Refund amount exceeds the remaining refundable balance for this order.',
            ], 422);
        }
        ...
        $adminNote = $orderReturn->admin_note;
...
public function refundPartial(
   ...
   private function processRefund(
    ...
    try {
        ...
        $vendorFinanceService->recordRefund(
            $result['order'],
            (float) $result['refunded_amount'],
            (string) $result['refund_reference']
        );
