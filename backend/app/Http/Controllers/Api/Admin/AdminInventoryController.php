...
class AdminInventoryController extends Controller
{
    public function index(Request $request): JsonResponse
    {
        ...
        $inventory->getCollection()
            ->transform(function ($row) use (
                ...
                $threshold = (int) (
                    $row->low_stock_threshold ?? 10
                );

                $trackQuantity = (bool) (
                    $row->track_quantity ?? true
                );
    ...
    private function getInventoryStats(): array
    {
        ...
        return [
            'tracked_skus' =>
                (int) (
                    $summary->tracked_skus ?? 0
                ),

            'low_stock_skus' =>
                (int) (
                    $summary->low_stock_skus ?? 0
                ),

            'out_of_stock_skus' =>
                (int) (
                    $summary->out_of_stock_skus ?? 0
                ),

            'on_hand_units' =>
                (int) (
                    $summary->on_hand_units ?? 0
                ),
