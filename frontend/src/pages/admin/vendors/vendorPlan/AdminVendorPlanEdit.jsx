...
const AdminVendorPlanEdit = () => {
    ...
    const fetchPlan = async () => {
        try {
            ...
            if (!plan) {
                setGeneralError("Vendor plan not found.");
                return;
            }
            ...
            setFormData({
                ...
                billing: detectBillingType(plan),
                ...
                ai_authoring: Boolean(plan.ai_authoring),
            ...
            setGeneralError(error.response?.data?.message || "Unable to load vendor plan.");
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const response = await api.post(`/admin/vendor-plans/${id}/update`, payload);
            const plan = response.data?.plan;

            setSuccessMessage(response.data?.message || "Vendor plan updated successfully.");

            if (plan) {
                setFormData((previous) => ({
                    ...previous,
                    slug: plan.slug || previous.slug,
                    is_active: Boolean(plan.is_active),
                    is_default: Boolean(plan.is_default),
                }));
            }
            ...
            setGeneralError(error.response?.data?.message || "Unable to update vendor plan.");
        } finally {
            setSaving(false);
        }
    };
            ...
            <div className="mx-auto max-w-[1120px]">
                ...
                <div className="mb-[22px] flex items-center justify-between">
                    <div className="flex items-center gap-[9px]">
                        <h1 className="text-[22px] font-semibold text-[#111827]">Edit plan</h1>
                    ...
                    <div className="space-y-[20px]">
                        ...
                        <SectionCard title="Stripe">
                            ...
                            <FormField label="Product ID" error={errors.stripe_product_id?.[0]}>
                                ...
                                <FieldHelp>Stripe product identifier for this plan.</FieldHelp>
                            </FormField>
...
// Detect billing type
const detectBillingType = (plan) => {
    const hasMonthly = plan.monthly_price !== null && plan.monthly_price !== undefined;
    const hasYearly = plan.yearly_price !== null && plan.yearly_price !== undefined;

    const monthlyPrice = Number(plan.monthly_price || 0);
    const yearlyPrice = Number(plan.yearly_price || 0);

    if (hasMonthly && hasYearly && monthlyPrice === 0 && yearlyPrice === 0) {
        return "free";
    }
...
export default AdminVendorPlanEdit;